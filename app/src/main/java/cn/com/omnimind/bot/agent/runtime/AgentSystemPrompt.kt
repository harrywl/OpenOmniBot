    ...
    fun build(
        ...
        return when (locale) {
            PromptLocale.ZH_CN -> """
                ...
                - 用户消息中可能包含 `[用户上传图片描述]`，这是由 VLM 模型自动识别用户上传图片后生成的纯文本描述，你应将其视为图片内容本身进行理解，无需再尝试重新调用 `vlm_task` 查看该图片。
                - 设备自动化使用 `vlm_task`。
            ...
            PromptLocale.EN_US -> """
                ...
                - The user message may contain `[用户上传图片描述]` — this is a text description of a user-uploaded image automatically generated by a VLM model. Treat it as the image content itself; do not call `vlm_task` to re-recognize it.
